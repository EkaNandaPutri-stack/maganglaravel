    ...
    <main class="page">
        <section class="panel">
            ...
            <form action="{{ $formAction }}" method="POST" enctype="multipart/form-data">
                ...
                <input id="judul" name="judul" type="text" value="{{ old('judul', $berita->judul) }}" required>

                <label for="isi">Isi Berita</label>
                <textarea id="isi" name="isi" required>{{ old('isi', $berita->isi) }}</textarea>

                <label for="tanggal">Tanggal</label>
                <input id="tanggal" name="tanggal" type="date" value="{{ old('tanggal', $berita->tanggal?->format('Y-m-d')) }}" required>

                <label for="penulis">Penulis</label>
                <input id="penulis" name="penulis" type="text" value="{{ old('penulis', $berita->penulis) }}">
                ...
                @if ($berita->gambar)
                    <p>Foto saat ini: {{ basename($berita->gambar) }}</p>
                @endif

                <div class="actions">
                    <a class="button secondary" href="{{ route('admin.berita.index') }}">Batal</a>
                    <button class="button" type="submit">Simpan Berita</button>
                </div>
            </form>
        </section>
    </main>
